...
def Summation2PCF(data_file: str, rand_file: str, cfg_file: str, ell_max: int,
    ...
    for weight in weights:
        nDnR[weight] = (np.sum(data_full[weight]), np.sum(rand_full[weight]))

    wts1 = 'wtilde'
    wts2 = {0: 'w0', 2: 'w2', 4: 'wts'}
    ...
    for box_idx in range(total_boxes):
        ...
        if ftype == 'npy':
            ...
            B_ = ((nDnR[wts1][0]/nDnR[wts1][1]))*np.load(tempDirRand+'B_p{}_of_{}.npy'.format(
                box_idx+1,total_boxes))
        ...
        for s_idx in range(len(corr_dict['s'])):
            ...
            #     # Reaad in the B_1 convolved grid
            #     # Use ell=0 and RE kernel for B
            #     B1 = (nDnR[wts2[2]][0]/nDnR[wts2[2]][1])*u.fits_to_grid_unwrapper(
            ...
            for ell_idx in range(len(ell_step)):
                ...
                if ftype == 'npy':
                    ...
                    # Use ell=0 and RE kernel for B
                    B1ell = (nD/nR)*np.load(
                        tempDirRand+'B_{}_{}_0_RE_p{}_of_{}.npy'.format(
                    ...
                    # Reaad in the B_1 convolved grid
                    # Use ell=0 and RE kernel for B
                    B1well = (nDnR[wts2[2]][0]/nDnR[wts2[2]][1])*u.fits_to_grid_unwrapper(
