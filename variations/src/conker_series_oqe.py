        ...
        for s_idx in range(len(self.desired_s_bin_centers)):
            ...
            for ell_idx in range(len(self.ell_step)):
                ...
                self.D_g = np.histogramdd(data_XYZ,bins=self.grid_edges,
                                          weights=self.data[wts2[self.ell_step[ell_idx]]])[0]
                self.R_g = (self.nD/self.nR)*np.histogramdd(rand_XYZ,bins=self.grid_edges,
                                                            weights=self.rand[wts2[self.ell_step[ell_idx]]])[0]
