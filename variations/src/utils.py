...
def getFileString(file: str, file2=None):
    if file2==None:
        fileString = file.split('.fits')[0]
    else:
        fileString = file.split('.fits')[0].split('.')[0] +\
                                '_' + file2.split('.fits')[0]
    return fileString
  
def getLOSbox(rand_file: str, cfg_file: str, box_idx: int, rand_file2=None):
    ...
    rand_string = getFileString(rand_file, rand_file2)
    div_plan = fits.open('./divide/plans/'+rand_string+
            ...
            for k in range(kernel_shape[2]):
                if k_G[i,j,k] != 0:
                    azimuth_cell,polar_cell = phi_theta_leg(
                        kY[j],kZ[k],kX[i])
                    kvalue = sph_harm_y(n_, m_, polar_cell, azimuth_cell)
