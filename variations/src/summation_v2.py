...
def Summation2PCF(data_file: str, rand_file: str, cfg_file: str, ell_max: int,
                  ...
                  ftype : str,
                  use_oqe=False, use_moments=False, data_file2=None, rand_file2=None,
                  zrange=None):
    ...
    datafileString = u.getFileString(data_file, data_file2)
    randfileString = u.getFileString(rand_file, rand_file2)
    ...
    if use_moments:
        # if you use moments ftype must be npy
        ftype = 'fits'
        # also will need the bin centers and edges for histogramming gamma
        centers = np.load('./params/centers.npy')
        edges = np.load('./params/centers.npy')
    # else:
    # # otherwise try to load files to determine which ftype
    #     try:
    #     # Looks for two test files to get the correct ftype (data and rand)
    #         if use_oqe:
    #             btest = np.load(tempDirRand+'B_0_0_0_RE_w{}_p{}_of_{}.npy'.format(
    #                         ell_max, total_boxes, total_boxes))
    #             btest2 = np.load(tempDir+'W_0_{}_0_RE_p{}_of_{}.npy'.format(
    #                         ell_max, total_boxes, total_boxes))
    #         else:
    #             btest = np.load(tempDirRand+'B_0_0_0_RE_p{}_of_{}.npy'.format(
    ...
    with fits.open('./data/'+data_file) as f:
        data_full1 = f[1].data
        if zrange is not None:
            mask_data_full1 = (data_full1['z']>=zrange[0])&(data_full1['z']<=zrange[1])
            data_full1 = data_full1[mask_data_full1]
    with fits.open('./data/'+rand_file) as f:
        rand_full1 = f[1].data
        if zrange is not None:
            mask_rand_full1 = (rand_full1['z']>=zrange[0])&(rand_full1['z']<=zrange[1])
            rand_full1 = rand_full1[mask_rand_full1]

    if data_file2 == None:
        data_full2 = data_full1
        rand_full2 = rand_full1
    else:
        with fits.open('./data/'+data_file2) as f:
            data_full2 = f[1].data
            if zrange is not None:
                mask_data_full2 = (data_full2['z']>=zrange[0])&(data_full2['z']<=zrange[1])
                data_full2 = data_full2[mask_data_full2]
        with fits.open('./data/'+rand_file2) as f:
            rand_full2 = f[1].data
            if zrange is not None:
                mask_rand_full2 = (rand_full2['z']>=zrange[0])&(rand_full2['z']<=zrange[1])
                rand_full2 = rand_full2[mask_rand_full2]
    ...
    for weight in weights:
        nDnR_0[weight] = (np.sum(data_full1[weight]), np.sum(rand_full1[weight]))
        nDnR[weight] = (np.sum(data_full2[weight]), np.sum(rand_full2[weight]))
    ...
    s_values = np.linspace(cfg_set['sMin'],cfg_set['sMax'],cfg_set['sBinN'])
    corr_dict['s'] = s_values
    
    if not use_oqe:
        corr_dict['B'] = np.zeros(len(corr_dict['s']))
    else:
        for ell_idx in range(len(ell_step)):
            corr_dict['Bw'+str(ell_step[ell_idx])] = np.zeros(len(corr_dict['s']))
    # For each ell initialize W_ell
    for ell_idx in range(len(ell_step)):
        corr_dict['W'+str(ell_step[ell_idx])] = np.zeros(len(corr_dict['s']))

    if use_moments:
        # Initalize the moments dictionary for data storage
        moment_dict = {}
        
        # Set s and initialize B to 0
        moment_dict['s'] = s_values
        
        # For each ell initialize W_ell
        for ell_idx in range(len(ell_step)):
            moment_dict['mu1_'+str(ell_step[ell_idx])] = np.zeros(len(moment_dict['s']))
            moment_dict['mu2_'+str(ell_step[ell_idx])] = np.zeros(len(moment_dict['s']))
            moment_dict['mu3_'+str(ell_step[ell_idx])] = np.zeros(len(moment_dict['s']))
    ...
    try:
        os.makedirs('./correlations/'+
                    datafileString+'_'+cfg_file.split('.txt')[0])
    except:
        FileExistsError
    ...
    for box_idx in range(total_boxes):
        ...
        if not use_oqe:
            ...
            if ftype == 'npy':
                ...
                B_ = ((nDnR_0[wts1][0]/nDnR_0[wts1][1]))*np.load(tempDirRand+'B_p{}_of_{}.npy'.format(
                    box_idx+1,total_boxes))
                ...
                W_ = u.fits_to_grid_unwrapper(tempDir+'W_p{}_of_{}.fits'.format(
                    box_idx+1,total_boxes))
                B_ = ((nDnR_0[wts1][0]/nDnR_0[wts1][1]))*u.fits_to_grid_unwrapper(
        ...
        # Loop through each ell step 
            if use_oqe:
                wts1 = wts1_dict[ell_step[ell_idx]]
                ...
                if ftype == 'npy':
                    ...
                    # Read in W and B grids
                    W_ = np.load(tempDir+'W_w{}_p{}_of_{}.npy'.format(
                            ell_step[ell_idx], box_idx+1,total_boxes))
                    B_ = ((nDnR_0[wts1][0]/nDnR_0[wts1][1]))*np.load(tempDirRand+'B_w{}_p{}_of_{}.npy'.format(
                            ell_step[ell_idx], box_idx+1,total_boxes))
                    ...
                    # Read in W and B grids
                    W_ = u.fits_to_grid_unwrapper(tempDir+'W_w{}_p{}_of_{}.fits'.format(
                            ell_step[ell_idx], box_idx+1,total_boxes))
                    B_ = ((nDnR_0[wts1][0]/nDnR_0[wts1][1]))*u.fits_to_grid_unwrapper(
            ...
            # Loop through each s step
                ...
                if use_oqe:
                # if using oqe weights
                    wts2 = wts2_dict[ell_step[ell_idx]]
                    
                    if ftype == 'npy':
                    # If the user saved grids as npy files                        
                        # Read in the B_1 convolved grid
                        # Use ell=0 and RE kernel for B
                        B1well = (nDnR[wts2][0]/nDnR[wts2][1])*np.load(
                            tempDirRand+'B_{}_0_0_RE_w{}_p{}_of_{}.npy'.format(
                                s_idx,ell_step[ell_idx],box_idx+1,total_boxes))
                    ...
                    # If the user saved grids as fits files                        
                        # Read in the B_1 convolved grid
                        # Use ell=0 and RE kernel for B
                        B1well = (nDnR[wts2][0]/nDnR[wts2][1])*u.fits_to_grid_unwrapper(
                            tempDirRand+'B_{}_0_0_RE_w{}_p{}_of_{}.fits'.format(
                                s_idx,ell_step[ell_idx],box_idx+1,total_boxes))
                    if use_moments:
                        eta_rand = B_*B1well
                else:
                # then we are using just fkp weights                        
                    if ftype == 'npy':
                    # If the user saved grids as npy files
                        
                        # Read in the B_1 convolved grid
                        # Use ell=0 and RE kernel for B
                        B1ell = (nDnR[wts2][0]/nDnR[wts2][1])*np.load(
                    ...
                    # If the user saved grids as fits files
                        ...
                        # Use ell=0 and RE kernel for B
                        B1ell = (nDnR[wts2][0]/nDnR[wts2][1])*u.fits_to_grid_unwrapper(
                ...
                if use_oqe:
                    corr_dict['Bw'+
                              str(ell_step[ell_idx])][s_idx] += np.sum(B_*B1well)
                else:
                    if ell_idx==0:
                    # if using fkp weights only need ell=0 B counts for 2pcf
                        corr_dict['B'][s_idx] += np.sum(B_*B1ell)
                    # elif use_moments:
                    # # but if using moments still need to save the ell!=0 gammas
                    #     pass
                    else:
                    # if using fkp but not moments move onto next s bin
                        continue

                # if use_moments:
                # # if moments then save the gamma arrays. This will 
                # # likely get updated to save histograms rather than arrays
                #     eta_data = norm*W_*W1ell
                ...
                #     np.save('./moments/'+datafileString+
                #        '_'+cfg_file.split('.txt')[0]+'/'+'gamma_{}_{}_p{}_of_{}'.format(
                #                         s_idx,ell_step[ell_idx],
                #                         box_idx+1,total_boxes),
                #             counts)
    ...
    # Loop through each ell step
        ...
        if use_oqe:
            corr_dict['xi'+str(ell_step[ell_idx])] = corr_dict[
                'W'+str(ell_step[ell_idx])]/corr_dict['Bw'+
                              str(ell_step[ell_idx])]
        else:
            corr_dict['xi'+str(ell_step[ell_idx])] = corr_dict[
                'W'+str(ell_step[ell_idx])]/corr_dict['B']
        if use_moments:
        # actually calculate the moments here:
            for s_idx in range(len(moment_dict['s'])):     
                counts = []
                # Start file timer
                file_start_time = time.perf_counter()
                for box_idx in range(total_boxes):
                    counts.append(np.load('./moments/'+datafileString+
                                            '_'+cfg_file.split('.txt')[0]+'/'+
                                            'gamma_{}_{}_p{}_of_{}.npy'.format(
                                                s_idx,ell_step[ell_idx],
                                                box_idx+1,total_boxes)))
                # End file timer and update
                file_end_time = time.perf_counter()
                ...
                N = np.sum(counts)
                mu1_hist = (1/N)*np.sum(centers*counts)
                mu2_hist = (1/N)*np.sum((centers - mu1_hist)**2 *counts)
                mu3_hist = (1/N)*np.sum((centers - mu1_hist)**3 *counts)
                moment_dict['mu1_'+str(ell_step[ell_idx])][s_idx] = mu1_hist
                moment_dict['mu2_'+str(ell_step[ell_idx])][s_idx] = mu2_hist
                moment_dict['mu3_'+str(ell_step[ell_idx])][s_idx] = mu3_hist
    ...
    corr_table.writeto('./correlations/'+datafileString+
    ...
    if use_moments:
        ...
        for key in moment_dict.keys():
            ...
            # Write the data to a fits column and append
            moment_cols.append(fits.Column(
                name=key,array=moment_dict[key],format='E'))
            
        # Write the correlation data to a fits table
        moment_table = fits.BinTableHDU.from_columns(moment_cols)
        
        # Append the header with timing information
        # Keep file writing/reading time split between convolution and summation
        moment_table.header.set('T_DIVP',timing['T_DIVP'])
        moment_table.header.set('T_MAP',timing['T_MAP'])
        moment_table.header.set('T_CONV',timing['T_CONV'])
        moment_table.header.set('T_KERN',timing['T_KERN'])
        moment_table.header.set('T_FILE',timing['T_FILE'])
        moment_table.header.set('T_FILESM',timing['T_FILESM'])
        moment_table.header.set('T_SUM',timing['T_SUM'])
        
        # Write the corr pkg to file
        # Overwrite is fine here because the routine is fast
        moment_table.writeto('./moments/'+data_file.split('.fits')[0]+
                           '_'+cfg_file.split('.txt')[0]+'/2pcf_moments.fits',
                           overwrite=True)
    ...
    if plot_correlation == False:
        ...
        raise Exception('This option has not been updated and likely is outdated!')
...
# def Summation2PCF_temp_hist(data_file: str, rand_file: str, cfg_file: str, ell_max: int,
#                   plot_correlation: bool, save_plot: bool, verbose: bool,
#                   use_oqe=False, use_moments=False, data_file2=None, rand_file2=None):
#     # A function to sum over grids and compute the Legendre multipoles of 
#     #   the 2PCF
...
#                     W_ = np.load(tempDir+'W_w{}_p{}_of_{}.npy'.format(
#                             ell_step[ell_idx], box_idx+1,total_boxes))
#                     B_ = ((nDnR[wts1][0]/nDnR[wts1][1]))*np.load(tempDirRand+'B_w{}_p{}_of_{}.npy'.format(
#                             ell_step[ell_idx], box_idx+1,total_boxes))
...
#                     W_ = u.fits_to_grid_unwrapper(tempDir+'W_w{}_p{}_of_{}.fits'.format(
#                             ell_step[ell_idx], box_idx+1,total_boxes))
#                     B_ = ((nDnR[wts1][0]/nDnR[wts1][1]))*u.fits_to_grid_unwrapper(
#                         tempDirRand+'B_w{}_p{}_of_{}.fits'.format(
#                             ell_step[ell_idx], box_idx+1,total_boxes))
#                 # End file sum timer and update
#                 file_end_time = time.perf_counter()
...
#         if use_moments:
#         # actually calculate the moments here:
#             centers = np.load('./moments/temp_hist/centers.npy')
#             for s_idx in range(len(moment_dict['s'])):     
#                 counts = []
#                 for box_idx in range(total_boxes):
#                     counts.append(np.load('./moments/temp_hist/'+datafileString+
#                                             '_'+cfg_file.split('.txt')[0]+'/'+
#                                             'gamma_{}_{}_p{}_of_{}_hist.npy'.format(
#                                                 s_idx,ell_step[ell_idx],
#                                                 box_idx+1,total_boxes)))
#                 counts = np.sum(counts, axis=0)
#                 N = np.sum(counts)
...
#         # Write the corr pkg to file
#         # Overwrite is fine here because the routine is fast
#         moment_table.writeto('./moments/temp_hist/'+data_file.split('.fits')[0]+
#                            '_'+cfg_file.split('.txt')[0]+'/2pcf_moments_hist.fits',
#                            overwrite=True)
