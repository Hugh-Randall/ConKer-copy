    ...
    # Loop through each ell step
        ...
        # actually calculate the moments here:
            for s_idx in range(len(moment_dict['s'])):     
                gammas = []
                for box_idx in range(total_boxes):
                    gammas.append(np.load('./moments/'+datafileString+
                                            '_'+cfg_file.split('.txt')[0]+'/'+
                ...
                N = len(gamma)
                mu1 = np.mean(gamma)
                moment_dict['mu1_'+str(ell_step[ell_idx])][s_idx] = mu1
                moment_dict['mu2_'+str(ell_step[ell_idx])][s_idx] = np.var(gamma)
                moment_dict['mu3_'+str(ell_step[ell_idx])][s_idx] = (1/N)*np.sum((gamma-mu1)**3) 
