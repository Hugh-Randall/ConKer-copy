...
class ConKerBox:
    ...
    def __init__(self, data_file: str, rand_file: str, cfg_file: bool,
        ...
        for weight in weights:
            self.nDnR[weight] = (np.sum(data_full[weight]), np.sum(rand_full[weight]))
    ...
    def radialConvolveLegendre(self, ell_max: int, ftype: str = 'fits'):
        ...
        self.R_g_0 = (self.nDnR[wts1][0]/self.nDnR[wts1][1])*np.histogramdd(
        ...
        if ftype == 'fits':
            ...
            if self.rand_save:
                u.grid_to_fits_wrapper((self.nDnR[wts1][1]/self.nDnR[wts1][0])*self.R_g_0,self.mask,
            ...
            if self.rand_save:
                np.save(tempDirRand+'B_p{}_of_{}.npy'.format(
                    self.box_idx+1,self.total_boxes, wts1),
                    (self.nDnR[wts1][1]/self.nDnR[wts1][0])*self.R_g_0*self.mask)
        ...
        for s_idx in range(len(self.desired_s_bin_centers)):
            ...
            for ell_idx in range(len(self.ell_step)):
                ...
                w2_temp = wts2[self.ell_step[ell_idx]]
                self.D_g = np.histogramdd(data_XYZ,bins=self.grid_edges,
                                          weights=self.data[w2_temp])[0]
                self.R_g = ((self.nDnR[w2_temp][0]/self.nDnR[w2_temp][1]))*np.histogramdd(rand_XYZ,
                                                                                          bins=self.grid_edges,
                                                                                          weights=self.rand[w2_temp])[0]
                test = self.R_g
                # print(test.shape)
                test_mask = test[np.where(test>0)]
